# GroupWidgetVisualizer Gridstack Architecture


This document describes the current Flow architecture for `GroupWidgetVisualizer`, which uses Gridstack.js for layout rendering and treats the layout JSON stored in `GroupWidget` as the single source of truth for widget positioning.

## Introduction

The objective is to provide a dashboard renderer similar to the dashboards provided by ThingsBoard, using Gridstack.js as the underlying layout engine. The solution is based on a Web Component that integrates Gridstack.js on the client side and is wrapped by a Vaadin Flow server-side component.
