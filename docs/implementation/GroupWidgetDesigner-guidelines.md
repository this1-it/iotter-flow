# GroupWidgetDesigner Gridstack Architecture


## Introduction


This document describes the current Flow architecture for `GroupWidgetDesigner`, based on a Gridstack.js Web Component wrapped by Vaadin Flow.

The goal is a client-driven dashboard layout model compatible with ThingsBoard-style dashboards.

The dashboard configurator uses Gridstack.js as the underlying layout engine. The solution is based on a Web Component that runs on the client side and is wrapped by a Vaadin Flow server-side component.
