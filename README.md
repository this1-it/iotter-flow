...
Iotter-Flow is an enterprise-grade IoT monitoring platform that aggregates time-series data from distributed sensors through multiple protocols (MQTT, REST), persists it in Cassandra, manages configuration through a relational database, and exposes a real-time Vaadin web interface with REST APIs.
