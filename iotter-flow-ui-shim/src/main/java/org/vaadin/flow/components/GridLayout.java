...
public class GridLayout extends FlexLayout {
    ...
    private static String getFlexValue(JustifyContentMode justifyContentMode) {
        try {
    ...
    public void setSpacing(Space spacing) {
    ...
    public void spanColumns(Component component, int span) {
    ...
    public void spanRows(Component component, int span) {
        Objects.requireNonNull(component);
        if (span < 1) {
            throw new IllegalArgumentException("Parameter span: invalid value " + span + ": must be 1 or greater");
        }
    ...
    public void startColumn(Component component, int start) {
        component.getElement().getStyle().set("grid-column-start", "span " + start);
    }

    public void startRow(Component component, int start) {
