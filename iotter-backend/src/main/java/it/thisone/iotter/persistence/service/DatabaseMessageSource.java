...
@Component
public class DatabaseMessageSource extends AbstractMessageSource implements Serializable {
	/**
	 * 
	 */
	private static final long serialVersionUID = -6016698946518695618L;

	private static Logger logger = LoggerFactory.getLogger(DatabaseMessageSource.class);
			...
			if (!templates.contains(template)) {
				try {
					...
					if (bundles.size() > 0) {
						...
						logger.error("template library may have been deleted / is going to be imported, no messages found with code {} {} ", template, code);
					}
				} catch (Throwable e) {
...
//		}
	...
	@PreDestroy
	public void destroy() {
		logger.debug("Callback triggered - @PreDestroy. {}",this.clear());
	}
