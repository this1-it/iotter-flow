package it.thisone.iotter.ui.uitask;
...
import it.thisone.iotter.config.Constants.LocalUiExecutor;
 ...
 *
 * https://github.com/mpilone/uitask-vaadin LoggingHandler.handleException
 * @author mpilone
 */
public class StrategyUIExecutor implements UIExecutor {
	private static final String NAMED_THREAD_FACTORY = "local-ui-%d";
	...
	/**
	 * Constructs the executor which will use a {@link CurrentUILocator}
	 * strategy to locate the UI.
	 */
	...
	/**
	 * Constructs the executor which will use the given strategy to locate the
	 * UI.
	 *
	 * @param uiLocator
	 *            the UI locator to use when locating the UI for access
	 *            synchronization
	 */
	 ...
	 *            the UI locator to use when locating the UI for access
	 *            synchronization
	 * @param executionPreparer
	 *            the preparer used to prepare a runnable for execution or null
	 *            for none
	 * @param executor
	 *            the executor to use for background task execution or null for
	 *            the default
	 */
	public StrategyUIExecutor(UILocator uiLocator,
	...
	/**
	 * Sets the UI locator to use when locating the UI for access
	 * synchronization. Must not be null.
	 *
	 * @param uiLocator
	 *            the UI locator
	 */
	public void setUiLocator(UILocator uiLocator) {
		if (uiLocator == null) {
			throw new IllegalArgumentException(
					"The uiLocator must not be null.");
		}
	 ...
	 *
	 * @param executionPreparer
	 *            the execution preparer or null for none
	 */
	...
	public Future<Void> access(Runnable runnable) {
		final UI ui = uiLocator.locateUI();
		UIRunnableFuture runnableFuture = new UIRunnableFuture(null, runnable,
				ui);
	...
	protected Executor getOrCreateExecutor() {
		if (executor == null) {
			...
			ThreadFactory namedThreadFactory = new ThreadFactoryBuilder().setNameFormat(ts + NAMED_THREAD_FACTORY).build();
			BlockingQueue<Runnable> queue = new LinkedBlockingQueue<Runnable>(LocalUiExecutor.QUEUE_CAPACITY);
		    ThreadPoolExecutor taskExecutor  = new ThreadPoolExecutor(
		    		LocalUiExecutor.CORE_POOL_SIZE, LocalUiExecutor.MAX_POOL_SIZE, LocalUiExecutor.KEEP_ALIVE, TimeUnit.SECONDS,
	 ...
	 * synchronization/locking. Therefore tasks can be submitted from any
	...
	public static class StaticUILocator implements UILocator {
		 ...
		 *
		 * @param ui
		 *            the UI to return on locate requests
		 */
	 ...
	 * attempt to improve responsiveness when waiting for a result while
	 * limiting unnecessary polling when there are no expected server side
	 * changes. If the UI is configured for push rather than polling, the
	 * polling interval will not be changed but the {@link UI#push()} method
	 * will be called when a task completes.
	 *
	 * @author mpilone
	 */
	public static class DynamicPollingManualPushUIPreparer implements
		...
		/**
		 * Constructs the preparer using the
		 * {@link #DEFAULT_NORMAL_POLL_INTERVAL} and
		 * {@link #DEFAULT_WORK_POLL_INTERVAL}.
		 */
		 ...
		 *            the poll interval when work is not pending and therefore
		 *            the client is not expecting a server generated change.
		 *            This interval is normally large (e.g. 15000 milliseconds)
		 *            to limit wasted network round trips.
		 * @param workPollInterval
		 *            the poll interval when work is pending and therefore the
		 *            client is expecting a server s generated change. This
		...
		public void prepareUI(UI ui, UIRunnableFuture runnableFuture) {
			...
			// We handle the start now even though the runnable may be getting
			// queued in
			// the executor. This is needed because we want any poll time
			// changes to the
			// UI to happen in the first client response otherwise the client
			// wouldn't
			// know that it should use the fast poll until the first slow poll
			// got the
			// change to the fast poll.
			taskStart(ui);

			// Add a complete listener so we can slow the polling down if all
			// background
			// work is done.
		...
		private UIRunnableFuture.CompleteListener createCompleteListener() {
			return new UIRunnableFuture.CompleteListener() {
				@Override
				public void uiRunnableComplete(
						UIRunnableFuture.CompleteEvent evt) {

					UIRunnableFuture taskRunnable = (UIRunnableFuture) evt
							.getSource();
		...
		/**
		 * Called by the complete listener to adjust the polling interval or
		 * perform a manual push.
		 *
		 * @param ui
		...
		private synchronized void taskFinish(UI ui) {
			...
			Integer pendingTasks = uiConnectorIdTaskCountMap.get(ui
					.getConnectorId());
			if (pendingTasks == null) {
				// This should never happen unless there is a bug.
				log.warn(
						"Pending task completed but no pending task count was "
				...
				if (pendingTasks == 0) {
					...
					uiConnectorIdTaskCountMap.put(ui.getConnectorId(),
							pendingTasks);
				}
			}
			...
			if (ui.getPushConfiguration().getPushMode() == PushMode.DISABLED
					&& pendingTasks == 0
					&& ui.getPollInterval() != normalPollInterval) {

				// Decrease the poll interval because there are no more running
				// tasks.
				ui.setPollInterval(normalPollInterval);
			} else if (ui.getPushConfiguration().getPushMode() == PushMode.MANUAL) {

				// Do a manual push because some background task just finished
				// and
			...
			log.debug("Background task finished. There are [{}] pending "
					+ "tasks for UI [{}] with a poll interval of [{}].",
		...
		private synchronized void taskStart(UI ui) {
			...
			int pendingTasks = uiConnectorIdTaskCountMap.containsKey(ui
					.getConnectorId()) ? uiConnectorIdTaskCountMap.get(ui
					.getConnectorId()) : 0;
			pendingTasks++;
