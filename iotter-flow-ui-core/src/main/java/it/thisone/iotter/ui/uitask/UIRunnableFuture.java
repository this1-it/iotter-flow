...
public class UIRunnableFuture implements UIRunnable, RunnableFuture<Void> {
	private static Logger logger = LoggerFactory
			.getLogger(UIRunnableFuture.class);

	/**
	 * The method to call on the {@link CompleteListener} when firing the
	 * complete event.
	 */
	...
	/**
	 * The active future that is running/will be run. When the future is
	 * complete, it must count down the {@link #doneLatch}.
	 */
	private Future<Void> future;

	/**
	 * The count down latch used to keep track of the number of futures that
	 * need to execute before this task is considered complete. Calls to
	 * {@link #get()} will block on this latch until it reaches 0.
	 */
	 ...
	 *            the runnable to execute in a background thread
	 * @param uiRunnable
	 *            the runnable to execute in the UI synchronized thread
	 * @param ui
	 *            the UI to synchronize work before updating any UI components
	 */
	...
	/**
	 * Adds a listener to be notified when the runnable is complete. The
	 * listener will always be notified in the UI lock. The complete event is
	 * fired if the runnable is canceled or c if it completes successfully.
	 * 
	 * @param listener
	 *            the listener to add
	 */
	...
	public void removeCompleteListener(CompleteListener listener) {
		eventRouter.removeListener(CompleteEvent.class, listener,
				COMPLETE_METHOD);
	}
	...
	/**
	 * Returns the UI runnable that this runnable future will execute. This may
	 * be null if no runnable was given.
	 * 
	...
	public void run() {
		...
		synchronized (futureMutex) {
			...
			if (future.isCancelled() || ui == null || !ui.isAttached()) {
				// Cancelled during doWork or the UI is no longer attached so
				// we'll skip
				// doUiUpdate and simply count down.
				doneLatch.countDown();
				fireComplete();
			} else {
				// Fire up runInUI runnable which gets us the second future of
				// the
				// task.
	...
	/**
	 * Fires the complete event after synchronizing with the UI to ensure that
	 * the event is handled in the UI lock.
	 */
	private void fireComplete() {
		ui.access(new Runnable() {
			...
			public void run() {
				if (ui.isAttached()) {
					eventRouter.fireEvent(new CompleteEvent(
							UIRunnableFuture.this));
				}
			}
		});
	}
	...
	public interface CompleteListener {
		/**
		 * Called when the runnable is complete, either because it was cancelled
		 * or because it finished execution.
		 * 
		 * @param evt
		 *            the event details
		 */
	 ...
	 * {@link UIRunnableFuture}. The {@link UIRunnableFuture#doWork()} method
	 * will be called and the {@link UIRunnableFuture#doneLatch} will be
	 * decremented.
	 * 
	 * @author mpilone
	 */
	private class RunInBackgroundRunnable implements Runnable {
		...
		public void run() {
			try {
				...
				logger.error("runInBackground", throwable);
			}
			finally {
				doneLatch.countDown();
			}
	...
	private class RunInUIRunnable implements Runnable {
		...
		public void run() {
			try {
				...
				logger.error("runInUI", throwable);
			}
			finally {
				doneLatch.countDown();
				fireComplete();
